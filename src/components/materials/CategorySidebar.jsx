...
export default function CategorySidebar({
  ...
  return (
    <aside className="rounded-2xl border border-[#e7eaf3] bg-white p-3 shadow-sm">
      ...
      <nav className="mt-2 space-y-0.5">
        ...
        {loading ? (
          ...
          <div className="rounded-lg bg-[#fafbff] px-3 py-3 text-center">
            <p className="text-xs text-[#7f88a6]">No categories yet</p>
            <button
              type="button"
              onClick={onCreate}
              className="mt-2 inline-flex items-center gap-1 rounded-lg border border-[#e3e6ef] bg-white px-2.5 py-1 text-xs font-semibold text-[#5f4ce6] transition hover:bg-[#f6f7fc]"
            >
              <FolderPlus className="h-3.5 w-3.5" /> Create one
            </button>
          </div>
        ) : (
          categories.map((c) => (
