      ...
      <div className="relative mx-auto grid w-full max-w-[1220px] grid-cols-1 items-center gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:gap-12 lg:px-10">
        <div>
          ...
          <div className="mt-10 flex flex-wrap gap-4">
            <button className="rounded-2xl bg-[#6e5cf7] px-7 py-3 text-base font-semibold text-white sm:px-8 sm:text-lg">
              Get Started Free
            </button>
            <button className="rounded-2xl border border-[#5b678a] px-7 py-3 text-base font-semibold text-white sm:px-8 sm:text-lg">
              See How It Works
            </button>
          </div>
        </div>
      ...
      <div className="mx-auto mt-10 grid w-full max-w-[1220px] grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {capabilityCards.map((item) => (
          <article key={item.title} className="rounded-3xl border border-[#e7eaf3] bg-white p-8 shadow-[0_10px_24px_rgba(27,39,94,0.06)]">
            <div className="mb-6 h-12 w-12 rounded-2xl bg-[#744fff]" />
...
function ProblemSection() {
  const cards = [
    "Manual Paper Creation Wastes Hours",
    "Zero Post-Exam Learning Feedback",
    "AI Cheating Goes Undetected",
  ]
  const texts = [
    "Teachers spend 8 to 12 hours crafting each exam from scratch, typing questions, formatting, and proofreading - time that could go toward actual teaching.",
    "Students receive only a numeric score. There is no explanation of what they got wrong, why it was wrong, or how to improve.",
    "With ChatGPT and other AI tools, students can generate perfect answers in seconds. Traditional plagiarism checkers cannot flag AI-written text.",
  ]
  return (
    <section id="challenge" className="scroll-mt-24 bg-[#081334] px-4 py-14 sm:px-6 sm:py-20">
      ...
      <div className="mx-auto mt-10 grid w-full max-w-[1220px] grid-cols-1 gap-6 lg:grid-cols-3">
        {cards.map((title, index) => (
          <article key={title} className="rounded-3xl border border-[#24335f] bg-[#162147] p-8">
            <div className="mb-6 h-12 w-12 rounded-2xl bg-[#1f2b58]" />
            <h3 className="text-2xl leading-tight font-semibold text-white sm:text-[28px]">{title}</h3>
            <p className="mt-4 text-sm leading-7 text-[#c2cae3] sm:text-base">{texts[index]}</p>
          </article>
        ))}
      </div>
    </section>
      ...
      <div className="mx-auto mt-10 grid w-full max-w-[1220px] grid-cols-1 gap-6 lg:grid-cols-3">
        {audienceCards.map((card) => (
          <article key={card.title} className="rounded-3xl border border-[#e7eaf3] bg-white p-8">
            <div className="mb-6 h-12 w-12 rounded-2xl bg-[#ebe8ff]" />
        ...
        <div className="grid grid-cols-1 gap-12 md:grid-cols-2 lg:grid-cols-4">
          <div>
            ...
            <div className="mt-6 flex gap-3">
              <div className="h-12 w-12 rounded-xl bg-[#273454]" />
              <div className="h-12 w-12 rounded-xl bg-[#273454]" />
            </div>
          </div>
          {columns.map((column) => (
